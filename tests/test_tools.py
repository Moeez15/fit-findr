...
def test_suggest_outfit_empty_wardrobe():
    ...
    assert isinstance(result, str)
    assert len(result.strip()) > 0   # general advice, not an exception/empty string
...
def test_create_fit_card_valid_input():
    item = search_listings("vintage graphic tee", size=None, max_price=50)[0]
    outfit = "Pair this with your baggy jeans and chunky sneakers."
...
def test_create_fit_card_empty_outfit():
    ...
    assert isinstance(card, str)
    assert len(card.strip()) > 0   # descriptive error string, not an exception
